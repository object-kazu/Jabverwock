...
#      @table = JWTable.new
    ...
    test 'caption title' do
      c = TableCaption.new.contentIs 'test'
      assert_equal c.tgStr, '<caption>test</caption>'
    end
...
#     ## table ##
    test "table" do
      t = JWTable.new
      assert_equal(t.tgStr, "<table>\n</table>")
    end

    test "table with id" do
      t = JWTable.new.attr(:id__sample)
      assert_equal(t.tgStr, "<table id=\"sample\">\n</table>")
    end
